...
If not already installed, `PetfinderR` can be installed through the usual means:
...
Now that `PetfinderR` is loaded, we can authenticate our connection to the API and begin extracting data! The authentication to the Petfinder API occurs when the `PetfinderR` class is initialized, which requires the API and secret keys we received in the previous step as parameters.
...
Hopefully this served as a useful introduction to the basic functionality of `PetfinderR` and some short examples to get started. The Petfinder database contains a wealth of adoptable animal information that is updated constantly so it is the goal of the `PetfinderR` wrapper library to make extracting and interacting with this data even easier whether you're building a web app or want to scrape the database for data analysis.
