...
The call returned a `data.frame` with 25 rows and 54 columns. We won't print the results here for the sake of brevity.
...
```{r}
petids <- random_records$id
pet_records <- pf$pet.get(petids)
pet_records
```
...
The call returned 15 cats with 43 dimensions, we skip printing the results for the sake of brevity.
...
```{r}
aby <- cats$cat.breeds[1]
aby_shelters <- pf$shelter.listByBreed('cat', aby)
aby_shelters
```
